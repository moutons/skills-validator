...
**US-001:** Parse scan subcommand As a user, I want `skills-validator scan` to be recognized as a valid command.

**US-002:** Enforce mutual exclusivity As a user, I want an error if I pass `--all --user` together so I know only one scope is allowed.

**US-003:** Default to --all As a user, I want `skills-validator scan` (no flags) to behave like `--all`.

**US-004:** Show help on unknown tool As a user, I want `--tool unknown` to show available tools so I can correct my input.
