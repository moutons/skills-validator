...
**US-001:** Human output matches validate command
As a user, I want scan output to look consistent with `validate` command output.

**US-002:** JSON includes summary
As a CI pipeline, I want JSON output with counts for programmatic processing.

**US-003:** Verbose shows all details
As a user, I want `--verbose` to show expanded paths and discovery details.

**US-004:** Non-verbose shows summary only
As a user, without `--verbose` I want only the overall summary and error list.
