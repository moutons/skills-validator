...
**US-001:** Find repo root from CWD As a user inside a git repo, I want the scanner to automatically detect the repo root.

**US-002:** Error outside repo As a user outside any git repo, I want `--repo` to fail with a clear message.

**US-003:** Handle subdirectories As a user in a nested subdirectory, I want repo detection to traverse up to find `.git`.
