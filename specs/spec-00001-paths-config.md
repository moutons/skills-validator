...
**US-001:** Load paths config from embedded JSONC
As a developer, I want the config loaded at startup so that tool paths are immediately available.

**US-002:** Normalize tool names to kebab-case
As a developer, I want "Claude Code" normalized to "claude-code" so that lookups are consistent regardless of input casing.

**US-003:** List all available tools
As a user, I want to see available tools when I pass an invalid tool name so I know what to use.
