...
**US-002:** Expand tilde As a user, I want `~/.claude/skills` to work identically to `$HOME/.claude/skills`.

**US-003:** Expand $REPO_ROOT
As a user, I want `$REPO_ROOT/.agent/skills` to resolve when running inside a git repo.

**US-004:** Handle missing repo root gracefully As a user, when `$REPO_ROOT` is in a template but no repo is detected, I want a clear error.
